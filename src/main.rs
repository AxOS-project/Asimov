...
#[derive(Parser)]
struct Args {
    /// Alias of Asimov, use hostname by default
    #[arg(long, env = "ASIMOV_ALIAS")]
    alias: Option<String>,

    /// Multicast address of Asimov
    #[arg(long, env = "ASIMOV_MULTIADDR", default_value = DEFAULT_MULTICAST)]
    multiaddr: Ipv4Addr,

    /// Port of Asimov
    #[arg(long, env = "ASIMOV_PORT", default_value_t = DEFAULT_PORT)]
    port: u16,

    /// Port of Asimov http server
    #[arg(long, env = "ASIMOV_HTTP_PORT", default_value_t = DEFAULT_HTTP_PORT)]
    http_port: u16,
...
struct ReceiveArgs {
    ...
    #[arg(long = "dest", env = "ASIMOV_DESTINATION", default_value = ".")]
    destination: PathBuf,
